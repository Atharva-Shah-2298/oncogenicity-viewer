...
const DATASETS = {
  ...
  gemini_pro: {
    label: "Gemini 3.1 Pro · 100 variants",
    gz: null,
    json: "gene_variant_dict_gemini_pro.json",
    desc: "Gemini 3.1 Pro biocuration with Google Search grounding · full model analysis per variant",
    isPrototype: true,
    isGemini: true,
  },
};
...
function renderStats() {
  ...
  for (const g of Object.values(DATA)) {
    ...
    for (const v of Object.values(g)) {
      const s = v.summary || {};
      const nr = s.n_references != null ? s.n_references : (v.references || []).length;
      nRef += nr;
      if (nr > 0) nWithEvidence += 1;
    }
  }
...
function inlineMd(s) {
  ...
  t = t.replace(
    /PMID:?\s*(\d{5,9})/gi,
    '<a class="pmid-inline" href="https://pubmed.ncbi.nlm.nih.gov/$1/" target="_blank" rel="noopener">PMID $1</a>'
  );
  t = t.replace(
    /(^|[\s(])(https?:\/\/[^\s<>")]+)/g,
    '$1<a href="$2" target="_blank" rel="noopener">link ↗</a>'
  );
  return t;
}
...
function loadGeminiVariant(gene, variant, entry) {
  const g = entry.gemini || {};
  const s = entry.summary || {};
  const consensus = s.consensus || "uncertain";
  ...
  let html = `
    <div class="variant-header">
      <div class="variant-title">
        ...
        <span class="model-tag">Gemini 3.1 Pro · grounded</span>
      </div>
      <div class="summary-grid">
        <span class="summary-pill"><strong>${pmids.length}</strong> PMID${pmids.length === 1 ? "" : "s"} cited</span>
        <span class="summary-pill"><strong>${sources.length}</strong> grounded source${sources.length === 1 ? "" : "s"}</span>
      </div>
    </div>`;
  ...
  if (!(g.response || "").trim()) {
    html += `
      <div class="empty-state inline-empty">
        <div class="empty-icon">⚠</div>
        <h3>No analysis returned</h3>
        <p>${escapeHtml(g.error || "The batch job returned no content for this variant.")}</p>
      </div>`;
    detail.innerHTML = html;
    return;
  }

  html += `<article class="gemini-analysis md-body">${renderMarkdown(g.response)}</article>`;

  if (sources.length) {
    html += '<div class="sources-block"><h4 class="sources-h">Grounded sources</h4><ul class="sources-list">';
    sources.forEach((src, i) => {
      const title = src.title || `Source ${i + 1}`;
      html += `<li><a href="${escapeHtml(src.uri)}" target="_blank" rel="noopener">${escapeHtml(title)} ↗</a></li>`;
    });
    html += "</ul></div>";
  }

  html += `
    <p class="model-disclaimer">Generated by Gemini 3.1 Pro with Google Search grounding.
    LLM output may contain errors — verify every PMID and claim against the primary source.</p>`;
...
function loadVariant(gene, variant) {
  ...
  if (entry.gemini) {
    loadGeminiVariant(gene, variant, entry);
    return;
  }
  const s = entry.summary || {};
